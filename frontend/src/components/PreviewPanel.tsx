...
const PreviwPanel = ({thumbnail,isLoading,aspectRatio} :
...
const onDownload = ()=>{
    ...
    return (
        ...
        <div className={`relative w-full max-w-xl min-h-[220px] overflow-hidden rounded-xl ${aspectClasses[aspectRatio]}`}>
            {/* Loading State */}
            {isLoading && (
            ...
            {!isLoading && thumbnail?.image_url && (
                <div className="group relative h-full w-full">
                    ...
                    <div className="absolute inset-0 flex items-end justify-center bg-black/10 opacity-0 transition-opacity group-hover:opacity-100">
                        <button onClick={onDownload} type="button" className="mb-6 flex items-center gap-2 rounded-md bg-white/30 px-5 py-2.5 text-xs font-medium ring-2 ring-white/40 backdrop-blur hover:scale-105 active:scale-95">
