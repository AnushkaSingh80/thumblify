...
import { ArrowUpRightIcon, DownloadCloudIcon, TrashIcon } from "lucide-react";
...
const MyGeneration = ()=> {
    ...
    const fetchThumbnails = async ()=>{
        //fetch thumbnails from backend here dummy for now as some thunbnails are stored in dummy data in src/assets/assets.ts
try {
    setLoading(true)
  ...
  try {
    ...
    return (
       ...
       <div className="mt-32 min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
        {/* Page Header */}
        <div className="mb-8">
            <h1 className="text-2xl font-bold text-zinc-20">My Generations</h1>
            <p className="text-sm text-zinc-400 mt-1">View and manage all your AI-generated thubnails</p>
        </div>
         {/*Loading State */}
         {loading && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{Array.from({length: 6}).map((_,i)=>(
                <div key={i} className="rounded-2xl bg-white/6 border border-white/10 animate-pukse h-[260px]"/> 
            ))}</div> 
         )}
         {/*Empty State */}
         {!loading && thumbnails.length===0 &&(
            <div className="text-center py-24">
                ...
                <p className="text-sm text-zinc-400 mt-2">Generate your first thubmnail to see it here</p>

            </div>
         )}
         {/*Grid*/}
         {!loading && thumbnails.length>0 &&(
            <div className="columns-1 sm:columns-2 lg:columns 2xl:columns-4 gap-8">
                             ...
                             <div className="p-4 space-y-2">
                                <h3 className="text-sm font-semibold text-zinc-100 line-clamp-2">{thumb.title}</h3>
                                 ...
                                 <div onClick={(e)=>e.stopPropagation()} className="absolute bottom-2 right-2 max-sm:flex sm:hidden group-hover:flex gap-1.5">
                                    ...
                                    <DownloadCloudIcon onClick={()=>handleDownload(thumb.image_url!)} className="size-6 bg-black/50 p-1 rounded hover:bg-pink-600 transition-all"/>
                                   {thumb.image_url && (
      <Link
    target="_blank"
